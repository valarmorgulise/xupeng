...
void Scheduler::start() {
  ...
  //     //m_rootFiber->swapIn();
  //     m_rootFiber->call();
  //     PENG_LOG_INFO(g_logger) << "call out " << m_rootFiber->getState();
  // }
}
...
void Scheduler::stop() {
  ...
  if (m_rootFiber) {
    ...
    //         true)); PENG_LOG_INFO(g_logger) << " root fiber is term, reset";
    //         t_fiber = m_rootFiber.get();
    //     }
    //     m_rootFiber->call();
    // }
  ...
  while (true) {
    ...
    bool is_active = false;
    {
      ...
      while (it != m_fibers.end()) {
        if (it->thread != -1 && it->thread != PENG::GetThreadId()) {
          ++it;
          tickle_me = true;
          continue;
        }
    ...
    if (ft.fiber && (ft.fiber->getState() != Fiber::TERM &&
      ...
      if (idle_fiber->getState() == Fiber::TERM) {
        PENG_LOG_INFO(g_logger) << "idle fiber term";
        break;
      }
...
void Scheduler::tickle() { PENG_LOG_INFO(g_logger) << "tickle"; }
