...
void HttpRequest::init() {
  ...
  if (!conn.empty()) {
    if (strcasecmp(conn.c_str(), "keey-alive") == 0) {
      m_close = false;
    } else {
      m_close = true;
    }
...
#define PARSE_PARAM(str, m, flag, trim)                                        \
  ...
  do {                                                                         \
    ...
    m.insert(std::make_pair(                                                   \
        ...
        PENG::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));     \
    if (pos == std::string::npos) {                                            \
      break;                                                                   \
    }                                                                          \
    ++pos;                                                                     \
...
void HttpRequest::initCookies() {
  ...
  PARSE_PARAM(cookie, m_cookies, ';', PENG::StringUtil::Trim);
}
...
std::string HttpResponse::setHeader(const std::string &key,
...
void HttpResponse::setCookiie(const std::string &key, const std::string &val,
                              time_t expired, const std::string &path,
                              const std::string &domain, bool secure) {
  std::stringstream ss;
  ss << key << " = " << val;
  if (expired > 0) {
    ss << ";expired = " << PENG::Time2Str(expired, "%a, %d %b %Y %H:%M:%S")
       << " GMT";
  }
  if (!domain.empty()) {
    ss << ";domain = " << domain;
